...
let ShipOnPiers = [new ShipOnPier([49 ,33], 33), new ShipOnPier([777], 2)]

let ports = [new Port('Rotterdam', 'Holland', 2234, 'Rijndwarsweg', [33]),
    new Port('Los Angeles', 'USA', 1124, 'Ways St', []),
    new Port('Jebel Ali', 'OAE', 5647, '940 Street', [])]

let ships = [
    new Ship('49', 49, 'Holland', 25, [
            new Log(12, new Date(2019, 10, 29), new Date(2020, 2, 17)),
                new Log(33, new Date(2020, 3, 30), new Date(2020, 7, 17))
            ]),

    new Ship('Delphine', 33, 'Great Britian', 10,
        [
            new Log(12, new Date(2019, 10, 29), new Date(2020, 2, 17)),
                new Log(2, new Date(2020, 3, 30), new Date(2020, 7, 17))
            ]),
    new Ship('Emma', 777, "Mexico", 2,
        [
            new Log(33, new Date(2017, 11, 30), new Date(2018, 2, 16)),
                new Log(2, new Date(2018, 5, 30), new Date(2019, 9, 17))])]

let piers = [new Pier('Stern', 33, 5), new Pier('Rikus', 12, 3),
    new Pier('Dunkan', 2, 1)]
...
app.post('/addPort', (req, res) => {
    ...
    if (!go) {
        ...
        res.render('pages/ports', {
            verify: 'Порт успішно доданий!', data: ports, dataShip: ships, dataPier: piers
            , allPiers: allPiers ,
            piers: AllMatherPiers
        })
        ...
        res.render('pages/workPort', {
            verify: 'Порт не доданий! Перевірте ЄДЕРПОУ', data: ports, dataShip: ships, dataPier: piers
            , allPiers: allPiers,
            piers: AllMatherPiers
        })
    }
...
app.post('/editPort', (req, res) => {
    ...
    res.render('pages/workPort', {
        data: ports, dataShip: ships, dataPier: piers, newText: `Порт успішно проредагованно!`
        , allPiers: allPiers,
        piers: AllMatherPiers
    })
...
app.post('/deletePort', (req, res) => {
    ...
    res.render('pages/workPort', {
        data: ports, dataShip: ships, dataPier: piers, newDelete: `Порт успішно видалено!`
        , allPiers: allPiers,
        piers: AllMatherPiers
    })
...
app.post('/findPort', (req, res) => {
    ...
    if (findPort !== undefined) res.render('pages/workPort', {
        ...
        data: ports, dataShip: ships, dataPier: piers, newFind: `Порт ${body.name} не Знайдено!`, allPiers: allPiers,
        piers: AllMatherPiers
    })
...
app.post('/addShip', (req, res) => {
    ...
    if (trust !== undefined) res.render('pages/workShip',
        {
            data: ports, dataShip: ships, dataPier: piers, verifyShip: `Корабель не додано! Перевірте номер корабля`
            , allPiers: allPiers,
            piers: AllMatherPiers
        })
    else {
        ships.push(newShip);
        res.render('pages/workShip', {
            data: ports, dataShip: ships, dataPier: piers, verifyShip: `Корабель успішно додано!`
            , allPiers: allPiers,
            piers: AllMatherPiers
        })
    }
...
app.post('/editShip', (req, res) => {
    ...
    res.render('pages/workShip', {
        newText: `Корабель успішно проредагованно!`, data: ports, dataShip: ships, dataPier: piers
        , allPiers: allPiers,
        piers: AllMatherPiers
    })
...
app.post('/deleteShip', (req, res) => {
    ...
    res.render('pages/workShip', {
        deleteText: `Корабель успішно видалено!`, data: ports, dataShip: ships, dataPier: piers
        , allPiers: allPiers,
        piers: AllMatherPiers
    })
...
app.post('/addPier', (req, res) => {
    ...
    res.render('pages/workPortsAndShips', {data: ports, dataShip: ships, dataPier: piers, allPiers: allPiers,
        piers: AllMatherPiers})
})
...
app.post('/time', (req, res) => {
    ...
    res.render('pages/report', {data: ports, dataShip: ships, dataPier: piers, allPiers: allPiers,
        show: show, info: info,
        piers: AllMatherPiers})
})
...
app.post('/allSP', (req, res) => {
    ...
    if (pier.length === 1){
        ...
        res.render('pages/report', {data: ports, dataShip: ships, dataPier: piers, allPiers: allPiers,
            piers: AllMatherPiers, hope: 'На даній пристані є кораблі', allShips: allShips, endShow: true})
    } else {
        res.render('pages/report', {data: ports, dataShip: ships, dataPier: piers, allPiers: allPiers,
            piers: AllMatherPiers, hope: 'На даній пристані немає кораблів'})
    }
})
...
app.post('/report', (req, res) => {
    ...
    if(allShips.length >= 1){
        res.render('pages/report', {data: ports, dataShip: ships, dataPier: piers, allPiers: allPiers,
            piers: AllMatherPiers, lastport: ports[indexPort].name, lastInfo: allShips, lastShow: true, lastChec: true})
    } else {
        res.render('pages/report', {data: ports, dataShip: ships, dataPier: piers, allPiers: allPiers,
            piers: AllMatherPiers, lastport: ports[indexPort].name, lastShow: false})
    }
